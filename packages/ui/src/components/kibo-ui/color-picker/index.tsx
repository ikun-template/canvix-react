...
export const ColorPicker = ({
  ...
  const [hue, _setHue] = useState(initialColor.hue());
  const [saturation, _setSaturation] = useState(initialColor.saturationl());
  const [lightness, _setLightness] = useState(initialColor.lightness());
  const [alpha, _setAlpha] = useState(initialColor.alpha() * 100);
  const [mode, setMode] = useState('hex');

  // Refs track latest values so notify always reads current state
  ...
  // Wrapped setters: update state + ref + schedule onChange
  ...
  // Sync from controlled value (no onChange — external source)
  useEffect(() => {
    if (value != null) {
      const color = Color(value);
      _setHue(color.hue());
      hueRef.current = color.hue();
      _setSaturation(color.saturationl());
      saturationRef.current = color.saturationl();
      _setLightness(color.lightness());
      lightnessRef.current = color.lightness();
      _setAlpha(color.alpha() * 100);
...
export const ColorPickerSelection = memo(
  ({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    useEffect(() => {
      ...
      const handlePointerMove = (e: PointerEvent) => applyPointer(e);
...
export const ColorPickerHue = ({
  ...
  return (
    ...
    >
      ...
      <Slider.Thumb className="border-border bg-background focus-visible:ring-ring block h-4 w-4 rounded-full border shadow transition-colors focus-visible:ring-1 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50" />
    </Slider.Root>
  );
};
