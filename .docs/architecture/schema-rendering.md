...
**UI = f(schema)**。所有渲染由 Schema 数据驱动，UI 组件不自持业务状态。变更流单向：Operation → Chronicle → 广播 → 渲染响应。
...
列表层通过 `PageLiveProvider` 的 `subscribe` prop 接收更新通知，仅关心集合变动（增删移）：

```tsx
// 编辑器中使用
const subscribePage = useCallback(
  (cb: () => void) =>
    chronicle.onUpdate(model => {
      if (model.target === 'page' && model.id === pageId) {
        cb();
        return;
      }
      if (model.target === 'document') {
        if (model.operations.some(op => op.chain[0] === 'pages')) cb();
      }
    }),
  [chronicle, pageId],
);
...
列表层渲染 widget 时通过 `WidgetLiveProvider` 注入 context，每个 widget 有独立的 `subscribe`：
...
每个 Widget 实例通过 `WidgetLiveProvider` 的 `subscribe` prop 独立订阅自身数据变更：

```tsx
// 编辑器中为每个 widget 创建过滤后的订阅
const subscribeWidget = useCallback(
  (cb: () => void) => chronicle.onUpdate((model) => {
    if (model.target === 'widget' && model.id === widgetId) cb();
  }),
  [chronicle, widgetId],
);

<WidgetLiveProvider widgetId={id} subscribe={subscribeWidget} ...>
  <WidgetShell />
</WidgetLiveProvider>
```

subscribe 触发时 Provider 内部 bump version → 消费者通过 `useWidgetLive()` 感知版本变化并重新读取数据。
...
<DocumentProvider value={docCtxValue}>
  <PageProvider value={pageCtxValue}>
    <RendererCtx.Provider value={...}>
      <WidgetProvider value={...}>        ← 根级 widget
        <WidgetShell />
      </WidgetProvider>
...
3. map 渲染子 widget，每个子 widget 注入新的 WidgetProvider（含 parentId、slotName）
...
3. Toolkit 组装 WidgetOperation: { target: 'widget', pageId, id, operations: [...] }
   │
4. Chronicle.update() 执行：就地修改 schema，计算逆操作，压入 history
   │
5. Chronicle 广播 OperationModel 到所有 onUpdate 监听者
   │
6. WidgetLiveProvider 的 subscribe 回调过滤出匹配的 widget → bump version
   │
7. WidgetShell 感知 version 变化
   │  chain: ['position', 'axis']
...
domains/
└── page-renderer/
    ├── package.json              # @canvix-react/page-renderer
    └── src/
